  ...
  return (
    ...
    >
      <div className="bg-white rounded-2xl shadow-xl relative border border-gray-100 overflow-visible">
        {/* 1. TABS - Now scrollable on mobile if needed */}
        ...
        <div className="flex flex-col md:flex-row">
          {/* FROM & TO GROUP (Mobile: Stacks / Desktop: Inline) */}
          <div className="flex flex-col sm:flex-row flex-[2] border-b md:border-b-0 md:border-r border-gray-100 relative">
            ...
            {/* SWAP BUTTON (Mobile: Absolute Right of From block / Desktop: Center) */}
            {tripType !== "hourly" && (
              <button
                onClick={swapCities}
                className="absolute right-4 top-[50%] -translate-y-[50%] sm:left-[50%] sm:-translate-x-[50%] z-20 bg-white border border-gray-200 shadow-md p-1.5 rounded-full text-blue-500 hover:scale-110 active:rotate-180 transition-all"
              >
          ...
          {/* DATE & TIME GROUP (Always Side-by-Side on Mobile) */}
        ...
        {/* 3. SEARCH BUTTON - More compact for mobile */}
        <div className="absolute left-1/2 -translate-x-1/2 -bottom-6 w-full flex justify-center px-6">
          <button className="bg-gradient-to-r from-blue-500 to-blue-700 text-white font-black text-xl sm:text-2xl px-10 sm:px-16 py-3 rounded-full shadow-lg hover:scale-105 active:scale-95 transition-all uppercase tracking-tighter w-full sm:w-auto">
            Search
          </button>
        </div>
      </div>
...
// --- SUB-COMPONENTS (With Mobile Sizing) ---
...
function PremiumCalendar({
  ...
  return (
    <div
      className="absolute top-full left-[-50%] sm:left-0 mt-2 w-[280px] sm:w-80 bg-white shadow-2xl rounded-xl z-[60] p-4 border border-gray-100"
      onClick={(e) => e.stopPropagation()}
    >
